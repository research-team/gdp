...
n(1) = an(Vm(1)) / (an(Vm(1)) + bn(Vm(1))); % potassium channel activation
m(1) = am(Vm(1)) / (am(Vm(1)) + bm(Vm(1))); % sodium channel activation
h(1) = ah(Vm(1)) / (ah(Vm(1)) + bh(Vm(1))); % sodium channel inactivation

for i=1:1:max-1
    % Forward Euler integration for n,m,h
    n(i+1) = n(i) + ((an(Vm(i)) * (1 - n(i))) - (bn(Vm(i)) * n(i))) * dt;
    m(i+1) = m(i) + ((am(Vm(i)) * (1 - m(i))) - (bm(Vm(i)) * m(i))) * dt;
    h(i+1) = h(i) + ((ah(Vm(i)) * (1 - h(i))) - (bh(Vm(i)) * h(i))) * dt;
...
print('-dpng','-r600', 'hh_neuron_result.png');
    ...
    % alpha: n, m, h
    function result = an(v)
        result = 0.01 * (v + 50) / (1 - exp(-(v + 50)/10));
    end

    function result = am(v)
        result = 0.1 * (v + 35) / (1 - exp(-(v + 35)/10));
    end

    function result = ah(v)
        result = 0.07 * exp(- (v+60) / 20);
    end

    % beta: n, m, h
    function result = bn(v)
        result = 0.125 * exp(- (v+60) / 80);
    end

    function result = bm(v)
        result = 4 * exp(- (v+60) / 18);
    end

    function result = bh(v)
